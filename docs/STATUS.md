...
**2026-09-20 (live activation):** 7 previously-unwired sources ingested live through the Python runner with authenticated telemetry (fetch/parse/reject counts, newest record date). Worker upsert now updates `canonical_url`; Hub payload carries the item's own URL and `eventDate`. Caveats: Ireland's 2 undated items are NEEDS_REVIEW (date unverified); Spain uses month-precision dates; TÜİK coverage limited to the home slider.
